...
const STEPS = [
  {
    ...
    fields: [
      ...
      { name: 'occupation', label: 'Ocupação', type: 'text', required: true, placeholder: 'Estudante, profissional...' },
    ],
  },
  {
    title: 'Gestão do Tempo',
    questions: [
      { id: 'q_overwhelm', label: 'Com que frequência você se sente sobrecarregado(a)?', options: ['Nunca', 'Às vezes', 'Frequentemente'], weights: [3, 2, 1] },
      { id: 'q_planning', label: 'Você costuma planejar seu dia ou semana?', options: ['Nunca', 'Às vezes', 'Sempre'], weights: [1, 2, 3] },
      { id: 'q_estimate', label: 'Você consegue estimar quanto tempo uma tarefa levará?', options: ['Não', 'Mais ou menos', 'Sim, com precisão'], weights: [1, 2, 3] },
    ],
  },
  {
    title: 'Gestão Financeira',
    questions: [
      { id: 'q_budget', label: 'Você mantém um orçamento mensal?', options: ['Não', 'Às vezes', 'Sim'], weights: [1, 2, 3] },
      { id: 'q_savings', label: 'Consegue poupar regularmente?', options: ['Não', 'Pouco', 'Sim, consistentemente'], weights: [1, 2, 3] },
      { id: 'q_track', label: 'Você sabe para onde seu dinheiro está indo?', options: ['Não faço ideia', 'Tenho uma noção', 'Sim, controlo cada gasto'], weights: [1, 2, 3] },
    ],
  },
  {
    title: 'Rotina e Hábitos',
    questions: [
      { id: 'q_peak', label: 'Em que período você se sente mais produtivo(a)?', options: ['Manhã', 'Tarde', 'Noite', 'Varia muito'], noWeight: true },
      { id: 'q_distractions', label: 'Quais são seus maiores ladrões de tempo?', type: 'multiselect', options: ['Redes sociais', 'Interrupções de colegas', 'Procrastinação', 'Falta de planejamento'], noWeight: true },
      { id: 'q_areas', label: 'Quais áreas da sua vida pessoal precisam de mais atenção?', type: 'multiselect', options: ['Saúde', 'Relacionamentos', 'Estudos', 'Finanças', 'Lazer'], noWeight: true },
    ],
  },
]
...
export default function QuestionnaireWizard() {
  ...
  const handleFieldChange = (name, value) => setResponses(prev => ({ ...prev, [name]: value }))
  const handleQuestionChange = (id, value) => setResponses(prev => ({ ...prev, [id]: value }))

  const canProceed = () => {
    const currentStep = STEPS[step]
    if (currentStep.fields) return currentStep.fields.every(f => !f.required || responses[f.name])
    if (currentStep.questions) return currentStep.questions.every(q => q.noWeight || responses[q.id])
    return true
  }
    ...
    STEPS.forEach(s => {
      s.questions?.forEach(q => {
        if (!q.noWeight && responses[q.id]) {
          ...
          if (idx !== -1 && q.weights) total += q.weights[idx]
        }
      })
    })
  ...
  const generateGoals = (level, responses) => {
    ...
    if (responses.q_distractions?.includes('Redes sociais')) goals.push('Reduzir uso de redes sociais em 30 min/dia')
    return goals
  }
  ...
  const handleSave = async () => {
    ...
    if (responses.age) updateData.age = responses.age
    if (responses.occupation) updateData.occupation = responses.occupation
                  ...
                  {q.options.map(opt => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        const newVal = prev.includes(opt) ? prev.filter(v => v !== opt) : [...prev, opt]
